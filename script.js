...
addButton.addEventListener("click", (event) => {
  for (let index = 0; index < arrObj.length; index++) {
    const res = {
      ...
      status: `${arrObj[index].status}`,
    };
    listWrap[0].innerHTML += `<div class="selected__info" p-name = "${res.name}" div-name = "selected__div"><p p-name = "${res.name}">"id" ${res.id} </p><p p-name = "${res.name}"> "name" ${res.name}</p><p p-name = "${res.name}">"status" ${res.status}</p><button btn-name ="delete">delete</button></div>`;
  }
});
